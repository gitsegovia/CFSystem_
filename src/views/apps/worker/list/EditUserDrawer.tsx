...
const SidebarEditUser = (props: SidebarAddUserType) => {
    // ** Props
    const { open, toggle, update, infoWorker } = props
    ...
    const {
        ...
        defaultValues: {
            email: infoWorker.email,
            firstName: infoWorker.firstName,
            lastName: infoWorker.lastName,
            idnDni: infoWorker.idnDni,
            phone: infoWorker.phone,
            address: infoWorker.address,
            gender: infoWorker.gender,
            condition: infoWorker.condition,
        },
        mode: "onChange",
    ...
    const onSubmit = (dataUser: UserData) => {
        dispatch(
            addUser({
                mutate,
                query,
                data: { ...dataUser, id: infoWorker.id },
                callback: (data, error) => {
                    if (data && data === true) {
                        toggle()
                        if (update) {
                            update({ ...infoWorker, ...dataUser })
                        }
                    } else {
                        toast.error(error)
                    }
    ...
    return (
        <Drawer open={open} anchor="right" variant="temporary" onClose={handleClose} ModalProps={{ keepMounted: true }} sx={{ "& .MuiDrawer-paper": { width: { xs: 300, sm: 400 } } }}>
            <Header>
                <Typography variant="h6">Editar obrero</Typography>
            ...
            <Box sx={{ p: 5 }}>
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <FormControl fullWidth sx={{ mb: 6 }}>
                        <InputLabel id="condition-select">Grado</InputLabel>
                        <Controller
                            name="condition"
                            control={control}
                            rules={{ required: true }}
